...
export default function HomePage() {
  ...
  if (!isClient || loading) { return <p className="text-center mt-32 text-slate-500">Carregando dashboard...</p>; }
  if (error) { return <p className="text-center mt-32 text-red-500">Erro: {error}</p>; }
  ...
  const getLatestKpiData = (kpiName: string) => {
    const kpiData = kpisFiltrados.filter(k => k.KPI === kpiName).sort((a, b) => {
        if (!b.COMPETÊNCIA) return -1; if (!a.COMPETÊNCIA) return 1;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <nav className="mb-8">
          <div className="flex space-x-2 sm:space-x-4 border-b border-slate-200 overflow-x-auto pb-px">
            {times.map(time => (<button key={time} onClick={() => setTimeSelecionado(time)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {kpisParaCartoes.map(nomeDoKpi => {
            ...
            return (<KpiCard key={kpiData.KPI} titulo={kpiData.KPI} valor={formatarValor(kpiData.RESULTADO, kpiData.GRANDEZA)} descricao={`Competência: ${kpiData.COMPETÊNCIA}`} />);
          })}
        </div>
        {kpisFiltrados.length === 0 && !loading && (<div className="bg-white p-8 rounded-2xl shadow text-center text-slate-500"><p>Nenhum KPI encontrado para o time "{timeSelecionado}".</p></div>)}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {kpisUnicosDoTime.map(nomeDoKpi => (<div key={nomeDoKpi} className="bg-white p-6 rounded-2xl shadow-lg"><GraficoDeBarras data={kpisFiltrados} kpiAlvo={nomeDoKpi} eixoXDataKey={"COMPETÊNCIA"} /></div>))}
        </div>
      </div>
    </main>
  );
